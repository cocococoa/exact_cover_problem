...
private:
    struct HNode
    {
        ...
        {
            std::stringstream ss;
            ss << "i: " << idx << ", NAME(i): " << name
               << ", LLINK(i): " << llink << ", RLINK(i): " << rlink;
            return ss.str();
        }
    };
...
public:
    ExactCoverProblemSolver(int i_num_items, const std::vector<std::vector<int>> &i_option_list)
    ...
    X5:
        ...
        else
        {
            ...
            level++;
            sol.emplace_back(xl);
            // std::cout << "[Push] Current solution: " << Tostr(sol.begin(), sol.end()) << std::endl;
            goto X2;
        }
    ...
    X8:
        ...
        else
        {
            ...
            xl = sol.back();
            sol.pop_back();
            // std::cout << "[Pop] Current solution: " << Tostr(sol.begin(), sol.end()) << std::endl;
            goto X6;
        }
    }
...
private:
    ...
    void PrintCurrentLink() const
    {
        ...
        for (const auto &hnode : hnode_list_)
            std::cout << hnode.str() << std::endl;
        std::cout << "Print vnode" << std::endl;
        for (const auto &vnode : vnode_list_)
            std::cout << vnode.str() << std::endl;
    }
    int MRV() const
    {
